### CAT regex assessment
### October 19, 2023

import re

# import raw logs from email as string literal
logs = r'''<86>May 28 08:21:45 okcjumperp01 sshd[7824]: Failed password for invalid user faketst from 10.1.1.150 port 11332 ssh2
...
# create empty list to store port #'s
ports = []

#regex's to account for each instance of port #'s in logs
regex = [r'port\s(\d+)',
         r'Port:\s(\d+)',
         r's_port:"(\d+)"',
         r'service:"(\d+)"']

# evaluate all regex on logs and collect findings
for x in regex:
    matches = re.findall(x,logs)
    ports.extend(matches)

# print results
print("Port numbers found in logs:\n")
print(ports)
